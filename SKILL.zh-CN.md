...
| 持续获客 | `vibe_subscribe` → 系统持续跟踪，候选自动积累，`vibe_leads` 领候选（领取时计费）→ 评分免费 → 相关线索进交付 |
...
- **Web 端是同一契约上的对等方，不是另一套世界**：用户可以在 https://vibedollar.net/app.html
  手动领取与判定 —— 同一套工具、同一把闸门、一条候选只判一次。人工判定回传的分值是
  `relevant=60` / `irrelevant=1`，所以你能把"人判的"和"你判的"区分开。**谁先领谁判**：
  若某条已被 Web 端判过，`vibe_submit_score` 会拒收该 id（`该候选已评分处理过 (不可重复评分)`）——
  **跳过该条、继续提交其余条目，不要重试、也不要当失败处理**。同理 `尚未领取` 表示这条还没被领取
  （先领再评），`已过期` 表示领取后 7 天未判：费用已产生、线索作废且永不再发 —— 所以"领了不评"
  比"不领"更糟。
- **只领你能处理的量**：**花钱的是领取，不是评分** —— 领了不评的线索同样占用额度，`pending` 队列到 50 条未评分时会暂停领取。
...
另外，pending 清单是**共享的**：Web 端（`app.html` → **待评分**）会把同一把 50 条闸门展示给用户，
两端谁先评完都算数。

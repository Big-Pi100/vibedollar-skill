...
- **可能有人在与你的 agent 并行判定**：Web 端（https://vibedollar.net/app.html → **待评分**）用同一套工具、同一把闸门领取与判定 —— 人工判定回传的分值是 `score: 60`（相关）/ `1`（无关），**谁先领谁判**；已被 Web 端判过的线索会被拒收（已评分处理过），**跳过该条、不要重试**。pending 队列（50 条未评分）是两端共享的：任一端评完即解锁下一批。

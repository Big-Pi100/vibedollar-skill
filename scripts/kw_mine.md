...
### 清单级扫描（2026-09-21 首跑）—— 它**首先是个筛 sub 的工具**

**何时跑**: 清单里有些 sub 只被少数关键词覆盖(说明我们不会说那伙人的话), 或想复核某个 sub 该不该留在清单。

**怎么跑**:
- 每个 sub 抽 **200 条 = 100 最新 + 100 最旧** (`vibe_sub_corpus(order="new")` + `order="old"`) ——
  只看最新会漏掉半年前的表达。实测整个清单 16 个 sub = **16 次调用 ≈ 4.8 万 token**, 语言季度级稳定,
  所以这是**季度级一次性活**, 不是每轮跑。
- 优先级由**覆盖矩阵**定: 先扫「被少数词命中」的 sub (首跑: AffiliateMarket 6 / startup 6 /
  Entrepreneur 8 / LeadGeneration 9 个词, 而 SideProject/SaaS/buildinpublic 是 17 个)。

**两个产出, 第二个更重要**:
1. **这伙人怎么说话** → 候选词;
2. **这伙人是不是 demand_side** → `audience_check` —— 首跑靠它直接判掉 2 个 sub。

**⚠️ 频次 ≠ 可用词 —— 落库前必须过两道闸** (首跑两个方向的候选都死在这两道闸上):
- **口径闸**: 语料里高频**不等于**是线索。实测 `co-founder` 命中 510 次, 但口径 NO 列表明确写着
  「找合伙人 / 找工作不算线索」—— 高频, 但它就是不该收。`solo founder`(844) 是自我标签不是需求。
- **容量闸**: 先算「这个词在清单内会命中多少」。实测 `product feedback` **3071** 条 —— 家族相似
  (`honest feedback` 实测 90% 精确) 但会把未领库存从 7,972 推到 **11k**, 远超 `quota_left 4164`
  → **记录待用, 不落库**。

**首跑结论 (4 个低覆盖 sub, 各 200 条分层抽样)**:

| sub | audience_check | 提词 | 处置 |
|---|---|---|---|
| `startup` | **YES** (~60% 现役早期创始人卡在首批用户) | 4 个 | 留 |
| `Entrepreneur` | PARTIAL (~15% 有产品, 主体是服务业小老板+泛生意闲聊) | **0 个** (明说提不出) | 留 |
| **`LeadGeneration`** | **NO** (卖 lead gen 服务的人 + 买线索的本地小生意) | 6 个(供给方行话) | **移出清单** |
| **`AffiliateMarket`** | **NO** (联盟客/流量供给侧 + program 招募方 + 刷单) | 5 个(偏弱) | **移出清单** |
